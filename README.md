...
| 기술 | 프로젝트 활용 방식 | 핵심 가치 |
|---|---|---|
| **KRX Open API · Python ETL** | KOSPI 종목 마스터와 일별 OHLCV를 거래일 단위로 수집하고, upsert·재시도·재개 로직으로 자동 적재 | 공식 원천 기반의 재현 가능한 데이터 파이프라인 구축 |
| **MySQL · SQLAlchemy** | 종목·시세·시장 지표·ETL 이력을 구조화해 저장하고 분석·예측 단계에 제공 | 대규모 시계열 데이터를 일관되게 관리 |
| **pandas · NumPy · SciPy** | RSI, MACD, 이동평균 괴리율, 변동성, 거래량·시장 피처를 만들고 EDA·가설검정을 수행 | 데이터에서 예측에 활용할 신호를 탐색·검증 |
| **scikit-learn · XGBoost · LightGBM** | Logistic Regression, Random Forest, XGBoost, LightGBM을 시간순 검증과 최종 홀드아웃으로 비교 | 정확도만이 아닌 ROC-AUC와 상위 20% 선별력을 기준으로 AI 모델 선택 |
| **KOSPI 시장 피처 결합** | 개별 종목 OHLCV에 KOSPI 수익률·변동성·이동평균·상대강도를 결합 | 시장 국면을 반영해 종목 상승확률 순위의 예측력 개선 |
| **Streamlit · Plotly** | 일일 예측 CSV, 상위 20개 종목, 개별 종목 상승확률을 조회하는 대시보드 구현 | 모델 결과를 사용자가 확인 가능한 의사결정 지원 화면으로 전환 |
| **OpenAI API · LLM 확장 구조** | 축적한 시장 데이터와 예측 결과를 자연어 리포트·챗봇 인사이트로 확장할 수 있도록 구성 | 정량 분석 결과의 해석과 전달 자동화 기반 마련 |

> 최종 모델은 111,927건의 미사용 홀드아웃에서 ROC-AUC 0.5800을 기록했고, 상승확률 상위 20% 종목의 실제 상승 비율은 전체 종목보다 **10.90%p 높았습니다.** 예측 결과는 투자 권유가 아닌 분석 참고용 순위로 제공합니다.
